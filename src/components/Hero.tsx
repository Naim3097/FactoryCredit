...
export default function Hero() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Build WhatsApp URL — user must tap a button to open it (iOS Safari blocks programmatic open after async fetch)
        ...
        if (waNumber) {
          ...
          setWaUrl(`https://wa.me/${waNumber}?text=${encodeURIComponent(message)}`);
        } else {
          setWaUrl("");
        }
              ...
              {submitStatus === "success" && (
                <div className="space-y-2.5">
                  <p className="text-xs text-accent-green">
                    Borang berjaya dihantar! Sila sahkan permohonan anda melalui WhatsApp.
                  </p>
                  {waUrl && (
                    <a
                      href={waUrl}
                      ...
                      className="inline-flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#1ebe57] active:bg-[#179347] text-white text-sm font-semibold py-2.5 px-4 rounded-lg transition-colors whitespace-nowrap"
                    >
